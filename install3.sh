...
GPG_PASS=$(openssl rand -base64 24) # separate passphrase for GPG encryption
...
if command -v gpg >/dev/null 2>&1; then
    gpg --batch --symmetric --cipher-algo AES256 --passphrase "$GPG_PASS" \
...
echo "$GPG_PASS" | sudo tee /opt/Prolter/.gpgpass > /dev/null
...
# Ensure .npm directory exists for prolteradmin
if [ ! -d "/home/prolteradmin/.npm" ]; then
    sudo -u prolteradmin mkdir -p /home/prolteradmin/.npm
    echo "📂 Created /home/prolteradmin/.npm directory"
else
    echo "ℹ️  /home/prolteradmin/.npm already exists"
fi

sudo chown -R prolteradmin:prolteradmin /opt/Prolter /home/prolteradmin/.npm
...
ProtectHome=true
ReadWritePaths=/opt/Prolter /home/prolteradmin
...
# Rate limiting config in its own file
cat <<EOF | sudo tee /etc/nginx/conf.d/ratelimit.conf > /dev/null
