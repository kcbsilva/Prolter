...
export default function SubscriberDashboardPage() {
  ...
  const churnRate = (
    (subscribersOverTime.reduce((acc, d) => acc + d.churned, 0) /
      (subscribersOverTime[0]?.total || 1)) *
    100
  ).toFixed(2);
    ...
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <List className="h-4 w-4" /> Subscribers List
          </Button>
        </div>
      </div>
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          <div className="relative w-full md:w-[360px]">
            ...
            <Input
              ...
              placeholder="Search subscribers, IDs, emails…"
              className="pl-9"
            />
          </div>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard
          ...
          subtitle={
            <span>
              <Badge className="mr-2 bg-[#233B6E]">Live</Badge> billed
            </span>
          }
        />
        <KpiCard
      ...
      <div className="mt-6 grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Card className="xl:col-span-2">
          <CardHeader className="pb-2">
            <CardTitle className="text-base">Growth & Churn</CardTitle>
          </CardHeader>
          <CardContent className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart
                data={subscribersOverTime}
                  ...
                  {overdueInvoices.map((o) => (
                    <TableRow key={o.id}>
                      ...
                      <TableCell
                        className={cn(
                          "text-right",
                          o.days >= 10 && "text-red-600 font-semibold"
                        )}
                      >
                        {o.days || "—"}
                      </TableCell>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <QuickStat
          icon={TrendingUp}
          label="Churn Rate (period)"
          value={`${churnRate}%`}
          hint="Total lost ÷ starting total"
        />
        <QuickStat
          icon={UserCheck}
          label="Net Adds (period)"
          value={`+${(
            subscribersOverTime.at(-1)!.total - subscribersOverTime[0]!.total
          ).toLocaleString()}`}
          hint="Gross adds − churn"
        />
        <QuickStat
          icon={Users}
          label="Avg. Actives"
          value={Math.round(
            subscribersOverTime.reduce((a, d) => a + d.active, 0) /
              subscribersOverTime.length
          ).toLocaleString()}
          hint="Mean active subs"
        />
      </div>
    </div>
  );
      ...
      <Card className={cn("overflow-hidden border", ring)}>
        <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
          ...
          <Icon
            className={cn(
              "h-5 w-5",
              accent === "warning" ? "text-[#FCA311]" : "text-[#233B6E]"
            )}
          />
        </CardHeader>
        <CardContent>
...
// — RecentSubscribers widget (embedded for now; extract to components/subscribers/RecentSubscribers.tsx if you prefer)
function RecentSubscribers() {
  interface MiniSubscriber {
    id: string;
    name?: string;
    full_name?: string;
    email?: string;
  ...
  const [rows, setRows] = React.useState<MiniSubscriber[]>([]);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/subscribers/list?perPage=5&page=1", {
        ...
        const list = Array.isArray(data)
          ? (data as MiniSubscriber[])
          : Array.isArray((data as any)?.subscribers)
          ? ((data as any).subscribers as MiniSubscriber[])
          : [];
        setRows(list);
      } catch {
        setRows([]);
